...
async def test_upload_backup_mirrors_and_flags(tmp_path, settings):
    s = _s3_settings(settings, tmp_path)
    directory = tmp_path / "a.com" / "20260611T030000Z"
    _make_backup_dir(directory, databases=["shop_db"])
    fake = FakeS3()
    await backup.upload_backup(fake, s, directory, "a.com", "20260611T030000Z")
...
async def test_download_backup_restores_local_copy(tmp_path, settings):
    ...
    fake = FakeS3()
    await backup.upload_backup(fake, s, source, "a.com", "20260611T030000Z")

    other_root = tmp_path / "elsewhere"
    target = await backup.download_backup(fake, s, str(other_root), "a.com", "20260611T030000Z")
    manifest = backup.read_manifest(target)
    assert manifest is not None
    backup.verify_checksums(target, manifest)

    with pytest.raises(NotFoundError):
        await backup.download_backup(fake, s, str(other_root), "a.com", "20260101T000000Z")
