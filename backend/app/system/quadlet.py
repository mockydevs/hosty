...
from app.domain.specs import ServiceSpec, StackSpec, derive_host_port, spec_hash
...
def build_env_file_path(tenant: str, stack: str, service: str) -> str:
    return f"{stack_dir(tenant, stack)}/env/{service}.build.env"
...
def container_unit(stack: StackSpec, service: ServiceSpec) -> str:
    image = f"hosty/{stack.name}-{service.name}:latest" if service.build_repo else service.image
    lines = [
        MANAGED_HEADER,
...
def dockerfile_build_unit(stack: StackSpec, service: ServiceSpec) -> str:
    ...
    if service.build_context != ".":
        workspace = f"{workspace}/{service.build_context}"
    ...
    lines = [
        ...
        f"PartOf={stack.name}-{service.name}.service",
        "",
        "[Service]",
        "Type=oneshot",
    ...
    if service.build_args:
        lines.append(
            f"EnvironmentFile={build_env_file_path(stack.tenant, stack.name, service.name)}"
        )
    ...
    for service in stack.services:
        ...
        if service.build_repo:
            if service.build_tool == "nixpacks":
                files[f"{stack.name}-{service.name}-nixpacks.service"] = nixpacks_build_unit(
                    stack, service
                )
            else:
                files[f"{stack.name}-{service.name}-build.service"] = dockerfile_build_unit(
                    stack, service
                )
            needs_git_sync = True
...
def env_files(stack: StackSpec) -> dict[str, str]:
    ...
    files.update(
        {
            build_env_file_path(stack.tenant, stack.name, service.name): "".join(
